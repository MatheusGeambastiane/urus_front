...
export function UserCard({ user, roleLabel, onClick }: UserCardProps) {
  ...
  return (
    <button
      ...
      className="group flex w-full min-w-0 items-center justify-between rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#c6a56b]/55 hover:border-white/30 lg:min-h-[76px] lg:rounded-[14px] lg:border-white/[0.065] lg:bg-black/20 lg:px-4 lg:hover:border-[#c6a56b]/25 lg:hover:bg-[#c6a56b]/[0.035]"
    >
      <div className="flex min-w-0 items-center gap-3">
        <div className="flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-full border border-white/10 bg-white/5 lg:h-11 lg:w-11 lg:rounded-[12px] lg:border-[#c6a56b]/15 lg:bg-[#c6a56b]/[0.055]">
          {user.profile_pic ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={user.profile_pic} alt={fullName} className="h-full w-full object-cover" />
          ) : (
            <UserRound className="h-6 w-6 text-white/70 lg:text-[#c6a56b]/70" />
          )}
        </div>
      ...
      <ChevronRight className="ml-3 h-4 w-4 shrink-0 text-white/30 transition group-hover:translate-x-0.5 group-hover:text-[#c6a56b]/70" />
    </button>
  );
}
