...
export function ResetPasswordForm({ isLoading = false }: ResetPasswordFormProps) {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!uid || !token) {
      setErrorMessage("Link invalido. Solicite uma nova redefinicao.");
      return;
    }
    ...
    try {
      const response = await fetch(
        `${env.apiBaseUrl}/dashboard/auth/password-reset/confirm/`,
        {
          method: "POST",
      ...
      if (!response.ok) {
        setErrorMessage(
          payload?.detail || "Nao foi possivel redefinir a senha."
        );
        return;
      }

      setStatusMessage(payload?.detail || "Senha redefinida com sucesso.");
    } catch {
        ...
        <div className="w-full rounded-[32px] border border-white/10 bg-white/5 p-6 shadow-[0_20px_60px_rgba(0,0,0,0.45)] backdrop-blur-xl">
          ...
          <form className="space-y-5" onSubmit={handleSubmit}>
            ...
            >
              ...
              {isSubmitting || isLoading ? "Atualizando..." : "Atualizar senha"}
            </button>
          </form>
